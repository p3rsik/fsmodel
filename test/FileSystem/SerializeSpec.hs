  ...
  describe "Serialize and deserialize SuperBlock" $ do
    it "decode . encode" $ do
      property $ \bs bc fb ic fi -> do
        ...
        decode (encode sb) `shouldBe` Just sb
  ...
  describe "Serialize and deserialize FileType" $ do
    it "decode . encode" $ do
      property $ \(ft :: FileType) -> do
        decode (encode ft) `shouldBe` Just ft
  describe "Serialize and deserialize FileStat" $ do
    it "decode . encode" $ do
      property $ \ft size -> do
        let fs = FS size ft
        decode (encode fs) `shouldBe` Just fs
  describe "Serialize and deserialize INode" $ do
    it "decode . encode" $ do
      property $ \bc ft size -> do
        let fs = FS size ft
        let bc' = if bc > 16 then 16 else bc
        let node = INode bc' fs $ replicate (fromIntegral bc') bc
        decode (encode node) `shouldBe` Just node
  describe "Serialize and deserialize Block" $ do
    it "decode . encode" $ do
      property $ \m' -> do
        let m = V.fromList m'
        decodeBlock (V.length m) (encodeBlock $ Block m) `shouldBe` Just (Block m)
